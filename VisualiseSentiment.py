...
GITHUB_CSV_URL = 'https://raw.githubusercontent.com/gaurav0003/SentimentAnalysis/refs/heads/main/crypto_data.csv'   # Replace with your actual raw GitHub CSV link
...
# --- Sidebar inputs ---
st.sidebar.header("Filter by datetime")
min_datetime = df['DateTime'].min()
max_datetime = df['DateTime'].max()

start_datetime = st.sidebar.datetime_input("Start datetime", value=min_datetime, min_value=min_datetime, max_value=max_datetime)
end_datetime = st.sidebar.datetime_input("End datetime", value=max_datetime, min_value=min_datetime, max_value=max_datetime)

# --- Filtered Data ---
filtered_df = df[(df['DateTime'] >= start_datetime) & (df['DateTime'] <= end_datetime)]

if filtered_df.empty:
    st.warning("No data available for the selected datetime range.")
    st.stop()
...
fig.text(0.5, 0.94, f"Data from {start_datetime.strftime('%d-%b-%Y %H:%M')} to {end_datetime.strftime('%d-%b-%Y %H:%M')}",
...
for i, session in enumerate(sessions):
    color = session_colors.get(session, '#f0f0f0')
...
fig.legend(handles=legend_handles, loc='upper right', title='Trading Sessions', fontsize=9, frameon=True, framealpha=0.95)
